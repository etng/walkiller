...
def notify_pp(title="", body="", template="html", token="", topic=""):
    ...
    if token:
        ...
        response = requests.post(
            "http://www.pushplus.plus/send?token=" + token,
            json={
                "title": title,
                "template": template,
                "topic": topic,
...
send_notify = partial(
    notify_pp, token=settings.pp_notify_token, topic=settings.pp_notify_topic
)
...
def notify(title, body, template=''):
    notifies.append(dict(title=title, body=body))


def send_notifies(task_name):
    from textwrap import dedent
    title = f"{settings.name} {task_name} 执行完毕"
    body = []
    if notifies:
        for notify in notifies:
            body.append(f'## {notify["title"]}')
            if notify["body"]:
                body.append("\n")
                body.append(f'{notify["body"]}')
        send_notify(title=title, body="\n".join(body), template="markdown")
    else:
        print("no notify yet")
...
def isTranslateServiceOnline(api_server):
    try:
        response = requests.post(
            f"{api_server}/api/v1/translator",
            json={
...
def _restart_nginx():
    ...
    with open(f"nginx.host.{settings.nginx_host}.conf", "w") as f:
        f.write(
            re.sub(
                prefix + r"(.*)" + suffix,
                lambda m: "\n".join([prefix, comment, upstreams, comment, suffix]),
                content,
                flags=re.DOTALL,
            )
        )
    ...
    notify(
        title=f"{settings.name}translate server nginx restarted",
        body="please check related port for availability",
    )


def _update_subscribe():
    result = subprocess.check_output(
        ["./ytoo_updater", "-url", settings.ytoo_subscribe_url]
    )
    proxies = []
    for line in result.splitlines():
        if line:
            ...
            try:
                ss_obfs_host = proxy.get("plugin", {}).get(
                    "obfs-host", settings.default_ss_obfs_host
                )
            except:
                pass
            proxies.append(
                {
                    ...
                    "ss_password": proxy.get(
                        "password", proxy.get("passwd", settings.default_ss_password)
                    ),
    ...
    logger.info(
        f"got {proxy_cnt} proxies, we have {len(settings.hosts)} hosts, so {proxy_per_ip} proxies per host"
    )
    if settings.max_proxy_per_ip and proxy_per_ip > settings.max_proxy_per_ip:
        proxy_per_ip = settings.max_proxy_per_ip
        logger.info(
            f"proxies per host {proxy_per_ip} > max proxies per host setting {settings.max_proxy_per_ip}, so update proxies per host to proxy_per_ip"
        )
        shuffle(proxies)
    offset = 0
    monitor_endpoints = []
...
def _sync_docker_images():
    ...
    if not len(hosts):
        logger.info(
            "no host other than localhost,skip image ETL(EXTRACT-TRANSFER-LOAD)"
        )
    else:
        for image, filename in settings.images.items():
            ...
            g.put(
                f"app/data/{filename}",
                os.path.join(settings.work_dir, "images", filename),
            )
    ...
    notify(title=f"docker images sync done", body=batch_tips)
...
def _update_docker_compose():
    ...
    for host in settings.hosts:
        ...
        with conn.cd(settings.work_dir):
            ...
            res = conn.put(
                ...
                os.path.join(settings.work_dir, "docker-compose.yml"),
            )
            try:
            ...
            try:
                conn.run(
                    "COMPOSE_HTTP_TIMEOUT=120 docker-compose up -d --remove-orphans"
                )
            except Exception as e:
    ...
    if len(failed_logs) > 1:
        body = "\n".join(failed_logs)
        template = "markdown"
    notify(title=f"service compose done", body=body, template="html")
...
def _test_endpoints():
    ...
    for endpoint in monitor_endpoints:
        ...
        logger.info(
            f"translate online: {showBool(translatorOnline)} proxy online: {showBool(proxyOnline)}"
        )
...
def update(c):
    ...
    _restart_nginx()
    _test_endpoints()
    send_notifies("update")


@task
def update_subscribe(c):
    _update_subscribe()
    send_notifies("update_subscribe")


@task
def sync_images(c):
    _sync_docker_images()
    send_notifies("sync_images")
