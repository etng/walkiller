    ...
    then
        ...
        then
            ...
            if [ -n "${SC_KEY}" ]; then
                ...
                curl -X POST https://sc.ftqq.com/${SC_KEY}.send --form "text=主人：项目代理挂啦${POLIPO_PORT}" --form "desp=${machine} ${now} 代理挂啦${POLIPO_PORT} ${SS_METHOD}://${SS_PASSWORD}@${SS_SERVER}:${SS_SERVER_PORT}"
            fi
